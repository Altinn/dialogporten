...
export default function () {
    ...
    describe('Search for dialogs including API-only dialogs', () => {
        ...
        // Verify that our API-only dialog is included in the results when ApiDialog = Either (default)
    ...
    describe('Search for dialogs excluding API-only dialogs', () => {
        let r = getSO('dialogs?ApiDialog=false&CreatedAfter=2023-01-01');
        expectStatusFor(r).to.equal(200);
        expect(r, 'response').to.have.validJsonBody();

        // Verify that our API-only dialog is excluded when ApiDialog = false
        let apiOnlyDialogId = dialogIds[0];
        let apiOnlyDialogFound = false;
