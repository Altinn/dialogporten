...
internal static class OrderExtensions
{
    ...
    {
        public IQueryable<T> ApplyOrder(IOrderSet<T> orderSet)
        {
            var first = true;
        ...
        private IQueryable<T> ApplyOrder(Order<T> order, bool first)
        {
            if (first)
            {
        ...
        public IQueryable<T> ApplyCondition(IOrderSet<T> orderSet, IContinuationTokenSet? continuationTokenSet)
        {
