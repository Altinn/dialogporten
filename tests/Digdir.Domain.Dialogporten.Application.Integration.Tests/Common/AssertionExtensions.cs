...
public static class ValidationErrorAssertionsExtensions
{
    public static void ShouldHaveErrorWithText(this ValidationError validationError, string expectedText) =>
        Assert.True(
            validationError.Errors.Any(e => e.ErrorMessage.Contains(expectedText, StringComparison.OrdinalIgnoreCase)),
...
public static class DomainErrorAssertionsExtensions
{
    public static void ShouldHaveErrorWithText(this DomainError domainError, string expectedText) =>
        Assert.True(
            domainError.Errors.Any(e => e.ErrorMessage.Contains(expectedText, StringComparison.OrdinalIgnoreCase)),
    ...
    public static void ShouldHaveErrorWithPropertyNameText(this DomainError domainError, string expectedText) =>
        Assert.True(
            domainError.Errors.Any(e => e.PropertyName.Contains(expectedText, StringComparison.OrdinalIgnoreCase)),
            $"Expected an error with {nameof(DomainFailure.PropertyName)} containing the text '{expectedText}'");
}
