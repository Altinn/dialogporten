...
public class CreateDialogAuthorizationContextTests(DialogApplication application) : ApplicationCollectionFixture(application)
{
    ...
    private static ChildAuthorizationContextDto CreateChildContext(Action<ChildAuthorizationContextDto>? modify = null)
    {
        var context = new ChildAuthorizationContextDto
        {
    ...
    [Fact]
    public Task Create_Should_Fail_When_ApiAction_Context_Is_Missing_Action() =>
        FlowBuilder.For(Application)
            .CreateSimpleDialog((x, _) =>
                x.AddApiAction(apiAction =>
                {
