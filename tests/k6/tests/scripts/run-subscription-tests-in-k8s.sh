...
help() {
    ...
    echo "  -v, --vus            Specify the number of client virtual users"
    echo "  -s, --svus           Specify the number of server (updaters) virtual users"
    echo "  -u, --users          Specify the number of total users to simulate"
    echo "  -w, --waittime       Specify the wait time between iterations"
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        -s|--svus)
            ...
            ;;
        -u|--users)
            number_of_endusers="$2"
            shift 2
            ;;
        -w|--waittime)
            sleep_between_updates="$2"
            shift 2
            ;;
        -d|--duration)
            duration="$2"
...
[ -z "$client_vus" ] && missing_args+=("vus (-v)")
[ -z "$server_vus" ] && missing_args+=("vus (-v)")
...
#-e SERVER_VUS=10 -e CLIENT_VUS=2000 -e DURATION=2m -e SLEEP_BETWEEN_UPDATES=0.5 -e NUMBER_OF_ENDUSERS=100

#archive_args="-e CLIENT_VUS=$client_vus -e DURATION=$duration -e SERVER_VUS=$server_vus" 
archive_args=()

# Always include these
archive_args+=( -e "CLIENT_VUS=$client_vus" )
archive_args+=( -e "DURATION=$duration" )
archive_args+=( -e "SERVER_VUS=$server_vus" )

# Conditionally add optional env vars
[[ -n "$sleep_between_updates" ]] && archive_args+=( -e "SLEEP_BETWEEN_UPDATES=$sleep_between_updates" )
[[ -n "$number_of_endusers" ]] && archive_args+=( -e "NUMBER_OF_ENDUSERS=$number_of_endusers" )
