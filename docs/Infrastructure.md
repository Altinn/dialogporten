...
- **Start a run:** Trigger the `Dispatch reindex dialogsearch` workflow. Select the environment, provide the janitor image tag (for example `1.23.4`), and pick the desired mode from the dropdown (`default`, `full`, `since`, `resume`, `stale-only`). Supply extra parameters such as the `since` timestamp, batch size, or worker count as needed. The workflow deploys the manual job definition, verifies that no execution is currently running, and then starts a new execution. The GitHub run finishes immediately after the start command to avoid long blocking runs; progress continues to stream from the container execution and can be inspected with the Azure CLI (`az containerapp job execution list/show/logs`) if needed.
- **Arguments:** The dispatch workflow exposes inputs mirroring the CLI switches (`--since`, `--stale-first`, `--batch-size`, `--workers`, `--throttle-ms`, `--work-mem-bytes`). Incompatible combinations (for example enabling stale-first without selecting the `stale-only` mode) are rejected before the job starts.
- **Avoiding overlap:** The reusable workflow aborts if it detects an execution with status `Processing` or `Running`. If a previous run was interrupted, use janitor's resumable flags (e.g. `--resume`) on the next invocation.
...
- **Check progress:** Trigger the `Show reindex dialogsearch progress` workflow to print the last 10 lines of stdout from the active execution. The step fails if no execution is currently running.
