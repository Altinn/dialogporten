...
public sealed class MetricsAggregationService
{
    ...
    public List<AggregatedMetricsRecord> AggregateFeatureMetrics(List<FeatureMetricRecord> rawMetrics)
    {
        ...
        // Map transaction types and filter out non-transaction feature metrics
        ...
        if (excludedCount > 0)
        {
            _logger.LogInformation("Excluded {ExcludedCount} non-transaction feature metrics from cost aggregation", excludedCount);
        }
