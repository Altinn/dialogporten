...
public class AuthorizationCheckBuilderTests
{
    ...
    public void GetAuthorizationChecksShouldBuildContextChecksForAllCarriers()
    {
        ...
        var dialogEntity = new DialogEntity
        {
            ...
            [
                new DialogAttachment
                {
                    ...
                    {
                        // The XACML action for attachments is always "read"; a stored action must be ignored
                        Action = "write",
        ...
        // Dialog attachment: action forced to read even though the stored context has "write"
        Assert.Contains(checks, c => c is
        {
        ...
        // Transmission: no explicit action, service resource override => defaults to read
        Assert.Contains(checks, c => c is
        {
            Action: Constants.ReadAction,
            Resource: { Kind: AuthorizationResourceSpecKind.Context, ServiceResource: "urn:altinn:resource:other-service" }
        ...
        // Transmission attachment: forced read on the additional resource attribute
        Assert.Contains(checks, c => c is
        {
            Action: Constants.ReadAction,
        ...
        // Navigational action: forced read on the overridden resource
        Assert.Contains(checks, c => c is
        {
            Action: Constants.ReadAction,
        ...
        string? serviceResource, string? additionalResourceAttribute, string expectedAction)
    {
        ...
        Assert.Equal(expectedAction, check.Action);
    }
