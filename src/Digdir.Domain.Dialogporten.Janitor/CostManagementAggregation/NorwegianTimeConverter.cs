...
public static class NorwegianTimeConverter
{
    ...
    /// <summary>
    /// Gets the start and end DateTime objects for a Norwegian date (for logging purposes).
    /// </summary>
    /// <param name="norwegianDate">The date in Norwegian timezone</param>
    /// <returns>Tuple containing start and end of day in Norwegian time</returns>
    public static (DateTime start, DateTime end) GetDayRangeInNorwegianTime(DateOnly norwegianDate)
    {
        var norwegianStartOfDay = norwegianDate.ToDateTime(TimeOnly.MinValue);
        var norwegianEndOfDay = norwegianDate.AddDays(1).ToDateTime(TimeOnly.MinValue).AddTicks(-1);

        return (norwegianStartOfDay, norwegianEndOfDay);
    }
}